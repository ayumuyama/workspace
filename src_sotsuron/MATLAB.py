...
from app_sotsuron import MATLAB_original_app as mat
...
if __name__ == "__main__":
    ...
    Fs, Cs, F, C, Decs, ErrorC, Error, MeanPrate, MembraneVar, T_size = mat.learning(
        dt, leak, epsr, epsf, alpha, beta, mu, Nneuron, Nx, Thresh, Fi, Ci
    )
    
    # ---------------------------------------------------------
    # 保存先ディレクトリの作成 (現在時刻のフォルダ)
    # ---------------------------------------------------------
    # フォーマット: YYYYMMDD_HHMMSS
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # results_sotsuron / 20260122_153000 のようなパスを作成
    base_save_dir = Path(__file__).parent.parent / "results_sotsuron"
    current_save_dir = base_save_dir / timestamp
    
    # フォルダを作成 (parents=Trueで親ディレクトリも必要なら作成、exist_ok=Trueで重複エラー回避)
    ...
    save_dir = Path(__file__).parent.parent / "results_sotsuron"
    save_dir.mkdir(parents=True, exist_ok=True)
    
    plt.tight_layout()
    plt.savefig(current_save_dir / "MATLAB.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(current_save_dir / "MATLAB_2.png")
    plt.close()