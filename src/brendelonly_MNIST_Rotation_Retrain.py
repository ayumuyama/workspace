...
if __name__ == "__main__":
    ...
    plt.plot(acc, label='SNN Accuracy (Online Learning)', color='blue')
    ...
    plt.title(f'Accuracy with Concept Drift & Retraining')
    ...
    # --- プロット2: ラスタープロット (Raster) ---
    # 画像単体で保存
    plt.figure(figsize=(15, 6))
    max_time = 500 * 30 * dt # 表示範囲 (最初の500サンプル分程度を表示)
    # 全データを表示すると重いので、重要な区間（0〜600サンプル程度）を表示
    display_samples = 1000
    display_time = display_samples * 30 * dt
    
    plt.scatter(spk_t, spk_i, s=2, c='black', marker='|', alpha=0.6)
    
    plt.axvline(x=RotationStart * 30 * dt, color='red', linestyle='--', linewidth=1.5, label='Rotation Start')
    plt.axvline(x=RetrainStart * 30 * dt, color='green', linestyle='-.', linewidth=1.5, label='Retrain Start')
    
    plt.title("Raster Plot (Drift & Adaptation)")
    ...
    # ファイル保存
    raster_path = os.path.join(save_dir, 'Raster.png')
    plt.savefig(raster_path)
    print(f"Saved raster plot to {raster_path}")
    plt.close()
