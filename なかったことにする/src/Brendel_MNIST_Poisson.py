...
def train_readout_mnist(F, C, X_data, y_data, Nneuron, Nx, dt, leak, Thresh, Gain, label_suffix=""):
    ...
    NumSamples = 500  # 【修正】サンプル数を増やして学習を安定させる
    Duration = 30      
...
if __name__ == "__main__":
    ...
    plt.plot(acc_poisson, label='Poisson Baseline (Rate Matched)', color='green', linestyle='--')
    ...
    # --- プロット2: ラスタープロット (拡大版) ---
    # 違いがわかりやすいように最後の少しだけを表示
    print("Plotting raster plots...")
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 8), sharex=True)
    
    max_time = 500 * 30 * dt
        
    ax1.scatter(spk_t_trained, spk_i_trained, s=5, c='blue', marker='|')
    ax1.set_title("Trained SNN: Structured & Sparse")
    ax1.set_ylabel("Neuron Index"); ax1.set_ylim(0, Nneuron)
    ax1.set_xlim(0, max_time)
    
    ax2.scatter(spk_t_poisson, spk_i_poisson, s=5, c='green', marker='|')
    ax2.set_title("Poisson Baseline: Random & Unstructured (Same Mean Rate)")
